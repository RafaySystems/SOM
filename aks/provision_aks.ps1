...
$azuretenantid = ''          # The Azure tenant ID  Example: 38d77531-9d10-484f-9c1d-3e3484cd9c79
$azuresubscriptionid = ''    # The Azure subscription ID.  Example: a2252eb2-7a25-432b-a5ec-e18eba6f2679
$azureclientid = ''          # The Azure client ID.  Example: 8932bf95-f213-4acc-99c8-f8eb5048c979
...
$time = get-date -format hh:mm:ss
write-host "$time - Checking if cluster exists" 

[string] $clusters = ..\rctl get cluster $clustername -o yaml
$rctlclustername = (($clusters -split "name:")[1] -split " ")[1]
if($rctlclustername -eq $clustername)
{
    $time = get-date -format hh:mm:ss
    write-host "$time - Cluster already exists" 
    
    #updating cluster
    $time = get-date -format hh:mm:ss
    write-host "$time - Updating cluster" 
